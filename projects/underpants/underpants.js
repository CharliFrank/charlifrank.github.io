...
_.reject = function(arr, test){
    let output = [];
    _.filter(arr, function(val, position, collection){if(test(val, position, collection) === false){
      output.push(val); 
    }});
    return output;
 };
...
_.unique = function(arr){
    let output = [];
    _.each(arr, function(val, pos, collection){
...
_.map = function(collection, test){
    let output = [];
    _.each(collection, function(val, position, collection) {
...
_.pluck = function(arr, property){
    let output = [];
    _.map(arr, function(val, position, collection){
...
_.extend = function(object1, object2){
    _.each(arguments, function(obj, position, collection){
